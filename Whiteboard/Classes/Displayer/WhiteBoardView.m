...
static NSString *WhiteYouTubeIframeLayoutFixScript(BOOL enableLogging) {
    NSString *script = [NSString stringWithFormat:@"(function(){"
           "try {"
           "var host = window.location.hostname || '';"
           "var path = window.location.pathname || '';"
           "var isYouTubeHost = /(^|\\.)youtube\\.com$/.test(host) || /(^|\\.)youtube-nocookie\\.com$/.test(host);"
           "if (!isYouTubeHost || path.indexOf('/embed/') !== 0) { return; }"
           "if (window.__netlessYouTubeLayoutFixInstalled) { return; }"
           ...
           "    if (payload) {"
           "      console.log('[WhiteYTLayoutFix]', JSON.stringify({ action: action, payload: payload }));"
           "    } else {"
           ...
           "function getVideo() { return document.querySelector('video.video-stream.html5-main-video'); }"
           ...
           "  if (!playerWidth || !playerHeight || !videoWidth || !videoHeight) { return null; }"
           ...
           "  var video = getVideo();"
           "  if (!player || !video || !player.isConnected || !video.isConnected) { return false; }"
           "  var expected = getExpectedBox(player, video);"
           "  if (!expected) { return false; }"
           "  var actual = getActualBox(player, video);"
           "  if (!boxNeedsCorrection(actual, expected)) { return false; }"
           ...
           "  var logSignature = ["
           "    actual.width, actual.height, actual.left, actual.top,"
           "    expected.width, expected.height, expected.left, expected.top,"
           "    expected.playerWidth, expected.playerHeight,"
           "    expected.videoWidth, expected.videoHeight"
           "  ].join('|');"
           "  var now = Date.now();"
           "  if (logSignature !== lastAppliedLogSignature || now - lastAppliedLogAt > repeatedLogInterval) {"
           "    lastAppliedLogSignature = logSignature;"
           "    lastAppliedLogAt = now;"
           "    log('applied', {"
           "      reason: reason,"
           "      actual: actual,"
           "      expected: { width: expected.width, height: expected.height, left: expected.left, top: expected.top },"
           "      player: { width: expected.playerWidth, height: expected.playerHeight },"
           "      video: { width: expected.videoWidth, height: expected.videoHeight }"
           "    });"
           "  }"
           "  return true;"
           "}"
           ...
           "  if (!isActive || domObserver || !window.MutationObserver || !document.documentElement) { return; }"
           ...
           "} catch (error) {"
           "  try {"
           "    console.warn('[WhiteYTLayoutFix] init-failed', error && error.message ? error.message : String(error));"
           "  } catch (_error) {}"
           "}"
...
#endif
    WKUserScript *script = [[WKUserScript alloc] initWithSource:WhiteYouTubeIframeLayoutFixScript(enableLogging)
